...
def main():
    ...
    parser.add_argument("--pos_scale", type=float, help="0.0到1.0之间的比例值", default=0.1)
    ...
    try:
        ...
        for i, joint_str in enumerate(joint_groups):
            print(f"\n=== 执行动作 {i+1}： joints={joint_str} ===")

            # 解析关节
            args.joints = joint_str
            joint_indices = [int(idx) for idx in args.joints.split(',')]
            ...
            # === 区间循环 ===
            action_end_step = (i + 1) * steps_per_action
            while running_step < action_end_step:
                running_step += 1
                ...
                if running_step % 100 == 0:
                    ...
                    print(
                        f"步数: {running_step}/{args.duration}, "
                    ...
                    for joint_idx in joint_indices:
                        ...
                        print(
                            f"  关节 {joint_idx}: 位置={current_pos:.3f} rad, "
                            f"状态={joint_state.anim_state}, "
                            f"速度={joint_state.speed:.3f} rad/s"
                        )
